...
func TestValidateBlockInvalidCommit(t *testing.T) {
	...
	t.Run("commit with unknown validator address", func(t *testing.T) {
		height := int64(3)
		valSet := state.LastValidators
		unknownAddr := ed25519.GenPrivKey().PubKey().Address()

		invalidCommit := &types.Commit{
			Height:     height - 1,
			Round:      0,
			BlockID:    state.LastBlockID,
			Signatures: make([]types.CommitSig, valSet.Size()),
		}

		for i := 0; i < valSet.Size(); i++ {
			_, val := valSet.GetByIndex(int32(i))
			privVal := privVals[val.Address.String()]
			...
			if i == 0 {
				vote.ValidatorAddress = unknownAddr
			}

			_, err := types.SignAndCheckVote(vote, privVal, chainID, false)
			require.NoError(t, err)
			invalidCommit.Signatures[i] = vote.CommitSig()
		}
